...
Vue.component('action-modal',{
  ...
  methods:{
    ...
    showModal(data, callback) {
      var actions = _.cloneDeep(data)
      if(actions.tags.serves.length == 0){
        actions.tags.serves = [ {value: [], option: 'first'} ]
      }
      if(actions.scenarios.serves.length == 0){
        actions.scenarios.serves = [
          { value: [],
            option: 'first',
            delivery: {
              timing: '',
  ...
  template:
  `<div>
    <a-modal :centered="true" v-model="visible" :width="600" :footer="null" :maskClosable="false" :destroyOnClose="true" :afterClose="afterClose">
    <div class="row justify-content-center pt-4">
      ...
      <div id="messagetarget-content" class="p-2">
          <component ref="comp" :is="currentComponent" :data="info" v-model:loading-count="loadingCount"></component>
      </div>
